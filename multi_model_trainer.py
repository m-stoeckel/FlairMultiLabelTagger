...
class MultiModelTrainer:
    def __init__(
        ...
        """
        self.models: Dict[str, flair.nn.Model] = models
        self.corpus: DynamicSingleLabelCorpus = corpus
    ...
    def train(
        ...
        for id, model in self.models.items():
            ...
            self.optimizers.update({id: optimizer})
        ...
        self.schedulers: Dict[str, torch.optim.Optimizer] = {}
        for id, optimizer in self.optimizers.items():
            ...
            self.schedulers.update({id: scheduler})

        self.train_subsets = {}
        for id, _ in self.models.items():
            train_data = self.corpus.get_train(id)
            self.train_subsets.update({id: train_data})

        # if training also uses dev data, include in training set
        if train_with_dev:
            for id, _ in self.models.items():
                train_data = ConcatDataset([self.train_subsets[id], self.corpus.get_dev(id)])
                self.train_subsets.update({id: train_data})
        ...
        try:
            ...
            train_losses = {id: 0 for id, _ in self.models.items()}
            ...
            for self.epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                for id, model in models:
                    log_line(log)
                    log.info(f"Training epoch for {id}")
                    log_line(log)

                    self.model = model
                    optimizer = self.optimizers[id]
                    train_data = self.train_subsets[id]

                    # if eval_on_train_shuffle:
                    #     train_part_indices = list(range(train_data))
                    ...
                    train_losses[id] = train_loss

                for id, model in self.models.items():
                    log_line(log)
                    log.info(f"Evaluating {id}")
                    self.model = model
                    optimizer = self.optimizers[id]
                    scheduler = self.schedulers[id]
                    train_data = self.train_subsets[id]

                    self.model.eval()

                    # anneal against train loss if training with dev, otherwise anneal against dev score
                    train_loss = train_losses[id]
                    current_score = train_loss

                    log_line(log)
                    log.info(
                        f"EPOCH {self.epoch} done: loss {current_score:.4f} - lr {learning_rate:.4f}"
                    )

                    # if self.use_tensorboard:  # FIXME: Fix tensorboard with multi models
                    #     writer.add_scalar("train_loss", current_score, self.epoch)
                    ...
                    if log_train:
                        ...
                        # depending on memory mode, embeddings are moved to CPU, GPU or deleted
                        store_embeddings(train_data, embeddings_storage_mode)
                    ...
                    if log_dev:
                        ...
                        # depending on memory mode, embeddings are moved to CPU, GPU or deleted
                        store_embeddings(self.corpus.get_dev(id), embeddings_storage_mode)
                    ...
                    if log_test:
                        ...
                        # depending on memory mode, embeddings are moved to CPU, GPU or deleted
                        store_embeddings(self.corpus.get_test(id), embeddings_storage_mode)
                    ...
                    if checkpoint and not param_selection_mode:
                        self.save_checkpoint(base_path / f"{id}-checkpoint.pt")
                    ...
                    if (
                        ...
                        self.model.save(base_path / f"{id}-best-model.pt")

                # if we do not use dev data for model selection, save final model
                if save_final_model and not param_selection_mode:
                    self.model.save(base_path / f"{id}-final-model.pt")
